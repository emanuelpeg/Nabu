...
@Component
class SchemaAdapterMySQL : SchemaAdapter {
    ...
    override fun generateCreateTableSql(table: Table): String {
        ...
        // En MySQL es común usar InnoDB por defecto, pero dejar la sintaxis estándar funciona perfectamente.
    ...
    override fun generateAlterColumnSql(tableName: String, oldColumnName: String, newColumn: Column): String {
        // Magia de MySQL: 'CHANGE' permite renombrar la columna Y cambiar su definición en una sola sentencia.
        val definition = buildColumnDefinition(newColumn)
        return "ALTER TABLE $tableName CHANGE $oldColumnName $definition;"
    }

    // --- Operaciones a nivel de Restricciones e Índices ---
    ...
    override fun generateDropConstraintSql(tableName: String, constraintName: String): String {
        // Nota: DROP CONSTRAINT es soportado en MySQL 8.0.16+.
        // Para versiones más antiguas, las FK se borran con DROP FOREIGN KEY.
    ...
    override fun generateDropIndexSql(tableName: String, indexName: String): String {
        // A diferencia de H2 o Postgres, MySQL requiere especificar la tabla al borrar un índice.
        return "DROP INDEX $indexName ON $tableName;"
    }


    private fun buildColumnDefinition(col: Column): String {
        val type = mapToMySQLType(col.type)
        val nullability = if (col.isNullable) "" else " NOT NULL"
        val autoInc = if (col.isAutoIncrement) " AUTO_INCREMENT" else ""

        // En MySQL, los strings como default deben ir entre comillas simples
        val defaultVal = col.defaultValue?.let {
            if (col.type == ColumnType.STRING || col.type == ColumnType.TEXT) " DEFAULT '$it'" else " DEFAULT $it"
        } ?: ""
    ...
    private fun mapToMySQLType(type: ColumnType): String {
        return when (type) {
            ColumnType.INTEGER -> "INT"
            ColumnType.DECIMAL -> "DECIMAL(19,4)"
            ColumnType.STRING -> "VARCHAR(255)"
            ColumnType.TEXT -> "TEXT"
            ColumnType.BOOLEAN -> "BOOLEAN" // MySQL internamente lo convierte a TINYINT(1)
            ColumnType.DATE -> "DATE"
            ColumnType.TIMESTAMP -> "DATETIME" // DATETIME es generalmente más seguro en MySQL que TIMESTAMP por temas de zonas horarias
    ...
    private fun mapReferenceAction(action: ReferenceAction): String {
        return when (action) {
            ...
            ReferenceAction.SET_DEFAULT -> "SET DEFAULT" // Nota: SET DEFAULT es parseado por MySQL pero a nivel de motor InnoDB suele actuar como RESTRICT.
        }
    }
}